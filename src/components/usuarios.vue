            ...
            <v-card-title>
                ...
                <v-tooltip top>
                    <template v-slot:activator="{ on }">
                        ...
                        >
                            <span class="mr-2">{{ !deletedLess ? 'Todas las cuentas de usuario' : 'Excluye cuentas expiradas' }}</span>
                            <v-spacer />
                ...
                <v-text-field
                    ...
                    :class="$vuetify.breakpoint.mdAndUp ? 'pt-6 col-4' : 'mt-4 col-6'"
                    outlined
                    dense
                />
            </v-card-title>
...
export default {
    ...
    data: () => ({
        ...
        headers: [
            ...
            { text: "Nombre Completo", value: "nombres", groupable: false, sortable: false },      
        ...
        async save () {
            ...
            // es un usuario nuevo
                ...
                if (resp.status == 200) {
                    ...
                    await this.getData()
                    this.$toastr("success", "Se ha creado el usuario correctamente", "Éxito!")
                }
                else if (resp.status == 409) {
                    this.saveDialog = false
                    this.$toastr("error", "El usuario ya existe en este sistema", "Error!")
                    return;
                }
                else {
            ...
            // se actualiza el usuario
                ...
                if (resp.status == 200) {
                    this.saveDialog = false
                    await this.getData()
                    this.$toastr("success", "Se ha actualizado el usuario correctamente", "Éxito!")
                }
                else {
                    console.log('error al guardar: ', resp)
                    this.saveDialog = false
